...
/**
 * Channel 을 유저로 생각하면 된다.
 * Channel에는 n개의 Song이 있고, Song의 channel은 Channel이다.
 * Channel에는 n개의 Playlist가 있고, Playlist의 owner는 Channel이다.
 * Playlist에는 n개의 Song이 있다.
 *
 */
...
// Array.from(document.querySelectorAll('#items')[7].querySelectorAll('button')).map(el =>
